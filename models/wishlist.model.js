...
const { singleByNameInChapter } = require("./course-content-detail.model");
...
module.exports = {
  ...
  async singleByCourseIdAndUsername(courseId, username) {
    const rows = await db.load(
      `select id from ${TBL_WISHLIST} where CourseId = '${courseId}' and username = '${username}'`
    );
    if (rows.length === 0) return null;
